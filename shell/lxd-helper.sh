...
install_btrfs() {
    msg "BLUE" "--- 检查并安装 BTRFS ---"
    if command -v btrfs &>/dev/null; then
        msg "GREEN" "BTRFS 工具已安装。"
        btrfs --version
        return 0
    fi

    msg "YELLOW" "未检测到 BTRFS 工具 (btrfs-progs)。准备通过APT安装..."
    read -p "$(msg "YELLOW" "确认安装 btrfs-progs 吗? [y/N]: ")" confirm
    if [[ ! "${confirm}" =~ ^[yY]$ ]]; then
        msg "BLUE" "操作已由用户取消。"
        return 1
    fi

    if ! sudo apt-get install -y btrfs-progs; then
        msg "RED" "通过 APT 安装 btrfs-progs 失败。"
        return 1
    fi
    
    msg "GREEN" "✓ btrfs-progs 安装成功。"
    return 0
}
...
create_btrfs_pool_from_device() {
    ...
    msg "RED" "警告: 此操作将格式化设备 '$device_path' 上的所有数据！"
    read -p "$(msg "YELLOW" "您确定要继续吗? [y/N]: ")" confirm
    if [[ ! "${confirm}" =~ ^[yY]$ ]]; then
        msg "BLUE" "操作已由用户取消。"
        return
    ...
    msg "BLUE" "步骤 1/1: 在 LXD 中创建 BTRFS 存储池..."
